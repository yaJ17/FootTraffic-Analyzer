#!/bin/bash
# YouTube streaming server startup script using OpenCV

# Clean up old processes
echo "Cleaning up old processes..."
pkill -f "python youtube_opencv.py" || true
pkill -f "python test_youtube.py" || true
sleep 1

# Start the YouTube streaming server with proper error logging
echo "Starting YouTube streaming server with OpenCV..."
cd flask_backend
echo "Starting YouTube server at $(date)" > ../youtube_server.log
python -u youtube_opencv.py >> ../youtube_server.log 2>&1 &
youtube_pid=$!
cd ..
